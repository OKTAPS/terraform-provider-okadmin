...
func resourceblacklistNetworkZone() *schema.Resource {
	return &schema.Resource{
		Create: resourceblacklistNetworkZoneCreate,
		Read:   resourceblacklistNetworkZoneRead,
		Update: resourceblacklistNetworkZoneUpdate,
		Delete: resourceblacklistNetworkZoneDelete,

		Schema: map[string]*schema.Schema{
			"network_zone_id": &schema.Schema{
...
type updateNetworkZone struct {
	useAsBlackList bool `json:"useAsBlackList,omitempty"`
}

func resourceblacklistNetworkZoneCreate(d *schema.ResourceData, m interface{}) error {
	return resourceblacklistNetworkZoneUpdate(d, m)
}
...
func resourceUpdateTechContactsRead(d *schema.ResourceData, m interface{}) error {
	url := fmt.Sprintf("https://%s-admin.%s/api/v1/zones/%s", m.(*Config).orgName, m.(*Config).domain, d.Get("network_zone_id"))
